...
class ConvNet2D_independent():
    def setup_model(self, X,
        ...
        model = models.Sequential([
            layers.Conv2D(filters = 32, kernel_size = 5, padding = "SAME", input_shape = (np.shape(X)[1],np.shape(X)[2],np.shape(X)[3])),
            ...
            layers.Conv2D(filters = 16, kernel_size = (5,3), strides = (1,2), padding = "SAME"),
            layers.BatchNormalization(),
            layers.Activation('relu'),
            ...
            layers.Conv2D(filters = 64, kernel_size = (5,3),  strides = (1,2), padding = 'SAME'),
            layers.BatchNormalization(),
            layers.Activation(tfa.activations.mish),
            ...
            layers.Conv2D(filters = num_of_pred_classes, kernel_size = (4,4), padding = "SAME"),
            layers.BatchNormalization(),
            #layers.Activation(GLU(bias = False, dim = , name='glu')),
